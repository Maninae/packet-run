...
- **Current phase: Phase 3** ([design/09-build-plan.md](design/09-build-plan.md)). Done: **DNS + caching** (lookup beat, TEST-NET-3 address, 8-run cache) and **the congestion bottleneck** (send-rate ladder, hidden capacity, slow-start/AIMD by feel — pipe rides the LONG road slot, ≤1 per run). Remaining, in rough build order: sniffer/encryption, undersea trench + satellite pass, Acts 1–3 as distinct biomes (palette families in design/08), the weather-state system, first Events, first elite (DDoS Swarm — spec in design/10). Method that works: TDD the engine mechanic → extend the economy sims → UI beat → E2E incl. one full-motion test → screenshot review → commit. **Kid playtests are non-negotiable from here** (Owen recruits ~3 kids, monthly).
