...
- **Aesthetic overhaul SHIPPED** (2026-07-03, Owen's "gorgeous, colorful, Mario Kart, 2.5D" go): (1) **Gate P1 PASSED** — Act-1 chase-cam mock through two fresh-reviewer rounds (TUNE→PASS, logged in design/README); mocks/road-act*.html set the per-biome art bar. (2) **The 2.5D road view** (design/11 step 3) behind `?view=road`: js/road/ (projection.js pure-math unit-tested, scenery.js act-1 layers, road-scene.js signposts=chips 1:1 with data-road-chip handles, road-view.js canvas formation + hop surge with the map's impact fates + Road⇄Map toggle). Act 1 + landscape only; portrait/later-acts/duel fall back to the classic map. tests/e2e/road.test.js incl. reduced- AND full-motion complete runs. (3) **Per-act sky backdrops** behind the classic map (css: --sky-* + --dest-glow per act; destination glow tracks the dock per orientation) + brightened wire tokens. (4) **DNS beat fix** (Owen: lookup felt pointless with a visible labeled dock): dock renders dim + "?" plate until the lookup; address plate (203.0.113.7, TEST-NET) after; cached runs show it from frame one; design/04 synced; tests/e2e/dns-view.test.js. Color invariants held everywhere: hazard amber / fragment cyan / Pip gold never shift.
